...
def crawler_futures(date: str, prod:str='TXF') -> pd.DataFrame:
    """期交所爬蟲"""
    ...
    if resp.ok:
        ...
        if Soup.find(class_='table_f') != None:
            ...
            foreign_investors = table_f("tr")[5]('td')[-2].text
            foreign_investors = int(re.sub(r"\r|\n|\t|,| ", "", foreign_investors))

            investment_trust  = table_f("tr")[4]('td')[-2].text
            investment_trust = int(re.sub(r"\r|\n|\t|,| ", "", investment_trust))
...
def main(start_date: str, end_date: str):
    ...
    for date in date_list:
        ...
        if len(df) > 0:
            ...
            try:
                df.to_sql(name='fut_contracts_date_tfx', con=db, if_exists='append', index=False)
