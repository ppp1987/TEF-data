...
def crawler_futures(date: str, prod:str='MTX') -> pd.DataFrame:
    """期交所爬蟲"""
    ...
    if resp.ok:
        ...
        if Soup.find(class_='table_f') != None:
            ...
            open_interest = table_f("tr")[-1]("td")[12].text
            open_interest = int(re.sub(r"\r|\n|\t|,| ", "", open_interest))
...
def main(start_date: str, end_date: str):
    ...
    for date in date_list:
        ...
        if len(df) > 0:
            ...
            try:
                df.to_sql(name='fut_daily_market_roprot_mtx', con=db, if_exists='append', index=False)
